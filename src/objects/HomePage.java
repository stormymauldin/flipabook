	...
	public static synchronized HomePage getInstance() {
		if (uniqueInstance == null) {
			...
			for (Entity temp_post : temp) {
				...
				// If a post is expired, delete it and all associated
				// conversations
//				System.out.println("Post time is " + temp_date);
				if ((new Date()).after(cal.getTime())) {
					...
					List<Entity> datastore_convos = datastore.prepare(query_convos)
	...
	public static synchronized void initialize() {
		// This will initialize all the datas from the datastore! It's probably
		// not that useful for appengine, but it's essential for debugging.
		...
		if (!init) {
			...
			for (Entity temp_post : temp) {
				...
				if (temp_date.after(cal.getTime())) {
					...
					Post post_obj = new Post(temp_flipabook_user, temp_title, temp_author, temp_isbn, temp_price,
					...
					System.out.println("User: " + temp_user.getEmail() + " has number of posts: "
							+ temp_flipabook_user.getNumCurrentPosts());
			...
			for (Entity conversation : convos) {
				...
				if (!foundPost) {
					...
					Post post_obj = new Post(temp_seller, temp_title, temp_author, temp_isbn, temp_price,
							temp_description, temp_date);
			...
			List<Entity> datastore_messages = datastore.prepare(message_query)
					.asList(FetchOptions.Builder.withLimit(100000));
